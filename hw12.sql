...
CREATE TABLE long as
  select name from tracks group by name having milliseconds > 360000;
...
CREATE TABLE long_album as
  select title from albums as a, tracks as t where a.albumId = t.albumId group by t.name having t.milliseconds > 360000;


-- Question 6
CREATE TABLE track_count as
  select albumId, count(albumId) from tracks group by albumId;


-- Question 7
CREATE TABLE album_count as
  select name, count(name) from artists as x, albums as y where x.artistId = y.artistId group by name;


-- Question 8
CREATE TABLE busiest_artists as
  select x.name, count(x.artistId) from artists as x, albums as y, tracks as z
	where x.artistId = y.artistId and y.albumId = z.albumId group by x.artistId;
